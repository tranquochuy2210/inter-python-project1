...
def load_neos(neo_csv_path = 'data/neos.csv'):
    ...
    with open(neo_csv_path, mode='r') as neo_file:
        csv_reader = csv.DictReader(neo_file)
        for row in csv_reader:
            new_object = NearEarthObject(row['pdes'], row['name'], row['diameter'], row['pha'])
...
def load_approaches(cad_json_path= 'data/cad.json'):
    ...
    with open(cad_json_path,'r') as cad_file:
        all_data = json.load(cad_file)
        
        for row in all_data['data']:
            new_object = CloseApproach(row[0], row[3], row[4], row[7])
            data.append(new_object)
    return data
